...
public class HullLibrary {
	...
	public boolean createConvexHull(HullDesc desc, HullResult result)
	{
		...
		ObjectArrayList<Vector> vertexSource = new ObjectArrayList<Vector>();
		MiscUtil.resize(vertexSource, vcount, Vector.class);

		Vector scale = Stack.alloc(new Vector(3));
		...
		if (ok) {
			//		if ( 1 ) // scale vertices back to their original size.
			{
				for (int i=0; i<ovcount[0]; i++) {
					Vector v = vertexSource.getQuick(i);
			...
			if (ok) {
				...
				ObjectArrayList<Vector> vertexScratch = new ObjectArrayList<Vector>();
				MiscUtil.resize(vertexScratch, hr.vcount, Vector.class);
				...
				if (desc.hasHullFlag(HullFlags.TRIANGLES)) { // if he wants the results as triangle!
					...
					MiscUtil.resize(result.outputVertices, ovcount[0], Vector.class);
	...
	private boolean computeHull(int vcount, ObjectArrayList<Vector> vertices, PHullResult result, int vlimit) {
	...
	private int calchull(ObjectArrayList<Vector> verts, int verts_count, IntArrayList tris_out, int[] tris_count, int vlimit) {
	...
	private int calchullgen(ObjectArrayList<Vector> verts, int verts_count, int vlimit) {
		...
		//int j;
		Vector bmin = Stack.alloc(verts.getQuick(0));
		Vector bmax = Stack.alloc(verts.getQuick(0));
	...
	private Int4 findSimplex(ObjectArrayList<Vector> verts, int verts_count, IntArrayList allow, Int4 out) {
		Vector tmp = Stack.alloc(new Vector(3)),
				tmp1 = Stack.alloc(new Vector(3)),
				tmp2 = Stack.alloc(new Vector(3));

		Vector[] basis = new Vector[/*3*/] {Stack.alloc(new Vector(3)), Stack.alloc(new Vector(3)), Stack.alloc(new Vector(3))};
		...
		if (p0 == p1 || (basis[0].get(X) == 0f && basis[0].get(Y) == 0f && basis[0].get(Z) == 0f)) {
			out.set(-1, -1, -1, -1);
			return out;
		}
		tmp.set(1f, 0.02f, 0f);
	...
	private boolean cleanupVertices(int svcount,
			ObjectArrayList<Vector> svertices,
			int stride,
			int[] vcount, // output number of vertices
			ObjectArrayList<Vector> vertices, // location to store the results.
			float normalepsilon,
			Vector scale) {
		...
		ObjectArrayList<Vector> vtx_ptr = svertices;
		int vtx_idx = 0;

		//	if ( 1 )
		{
			for (int i=0; i<svcount; i++) {
				Vector p = vtx_ptr.getQuick(vtx_idx);
		...
		if (dx < EPSILON || dy < EPSILON || dz < EPSILON || svcount < 3) {
			...
			float x1 = center.get(X) - dx;
			float x2 = center.get(X) + dx;

			float y1 = center.get(Y) - dy;
			float y2 = center.get(Y) + dy;

			float z1 = center.get(Z) - dz;
			float z2 = center.get(Z) + dz;
		...
		for (int i=0; i<svcount; i++) {
			...
			//		if ( 1 )
			{
				...
				for (j=0; j<vcount[0]; j++) {
					/// XXX might be broken
					Vector v = vertices.getQuick(j);

					dx = Math.abs(v.get(X) - px);
					dy = Math.abs(v.get(Y) - py);
					dz = Math.abs(v.get(Z) - pz);
					...
					if (dx < normalepsilon && dy < normalepsilon && dz < normalepsilon) {
						...
						float dist2 = getDist(v.get(X), v.get(Y), v.get(Z), center);
				...
				if (j == vcount[0]) {
					Vector dest = vertices.getQuick(vcount[0]);
					dest.set(X, px, false);
					dest.set(Y, py, false);
					dest.set(Z, pz, true);
					vcount[0]++;
	...
	private static Vector orth(Vector v, Vector out) {
		Vector a = Stack.alloc(new Vector(3));
		a.set(0f, 0f, 1f);
		a.cross(v, a);

		Vector b = Stack.alloc(new Vector(3));
		b.set(0f, 1f, 0f);
		b.cross(v, b);
	...
	private static int maxdirsterid(ObjectArrayList<Vector> p, int count, Vector dir, IntArrayList allow)
	{
		Vector tmp = Stack.alloc(new Vector(3)),
				tmp1 = Stack.alloc(new Vector(3)),
				tmp2 = Stack.alloc(new Vector(3)),
				u = Stack.alloc(new Vector(3)),
				v = Stack.alloc(new Vector(3));
	...
	private static Vector triNormal(Vector v0, Vector v1, Vector v2, Vector out) {
	...
	private static boolean above(ObjectArrayList<Vector> vertices, Int3 t, Vector p, float epsilon)
	{
		Vector n = triNormal(vertices.getQuick(t.getCoord(0)), vertices.getQuick(t.getCoord(1)), vertices.getQuick(t.getCoord(2)), Stack.alloc(new Vector(3)));
		Vector tmp = Stack.alloc(new Vector(3));
	...
	private static void addPoint(int[] vcount, ObjectArrayList<Vector> p, float x, float y, float z) {
		// XXX, might be broken
		Vector dest = p.getQuick(vcount[0]);
		dest.set(X, x, false);
		dest.set(Y, y, false);
		dest.set(Z, z, false);
		vcount[0]++;
	...
	private static float getDist(float px, float py, float pz, Vector p2) {
		float dx = px - p2.get(X);
		float dy = py - p2.get(Y);
		float dz = pz - p2.get(Z);
