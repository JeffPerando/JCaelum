...
class ClipPolygon
{
			...
			ObjectArrayList<Vector3f> clipped, int[] clipped_count)
	{
		...
		if (_classif != _prevclassif)
		{
			...
			vec_blend(clipped.getQuick(clipped_count[0]), point0, point1,
					blendfactor);
			clipped_count[0]++;
		}
		if (!_classif)
			...
			ObjectArrayList<Vector3f> clipped)
	{
		...
		// clip first point
		float firstdist = distance_point_plane(plane,
				polygon_points.getQuick(0));
		...
		for (int i = 1; i < polygon_point_count; i++)
		{
			...
			plane_clip_polygon_collect(polygon_points.getQuick(i - 1),
					polygon_points.getQuick(i), olddist, dist, clipped,
					clipped_count);
		...
		plane_clip_polygon_collect(
				polygon_points.getQuick(polygon_point_count - 1),
				polygon_points.getQuick(0), olddist, firstdist, clipped,
				clipped_count);
