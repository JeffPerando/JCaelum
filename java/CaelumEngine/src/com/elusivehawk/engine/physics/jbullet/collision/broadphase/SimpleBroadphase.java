...
public class SimpleBroadphase extends BroadphaseInterface
{
	...
	{
		assert (aabbMin.get(X) <= aabbMax.get(X)
				&& aabbMin.get(Y) <= aabbMax.get(Y) && aabbMin.get(Z) <= aabbMax
				.get(Z));
		
		SimpleBroadphaseProxy proxy = new SimpleBroadphaseProxy(aabbMin,
				aabbMax, shapeType, userPtr, collisionFilterGroup,
	...
	@Override
	public void setAabb(BroadphaseProxy proxy, Vector aabbMin, Vector aabbMax,
	...
	private static boolean aabbOverlap(SimpleBroadphaseProxy proxy0,
			SimpleBroadphaseProxy proxy1)
	{
		return proxy0.min.get(X) <= proxy1.max.get(X)
				&& proxy1.min.get(X) <= proxy0.max.get(X)
				&& proxy0.min.get(Y) <= proxy1.max.get(Y)
				&& proxy1.min.get(Y) <= proxy0.max.get(Y)
				&& proxy0.min.get(Z) <= proxy1.max.get(Z)
				&& proxy1.min.get(Z) <= proxy0.max.get(Z);
	}
			...
			for (int j = 0; j < handles.size(); j++)
			{
				...
				else
				{
					...
					if (!pairCache.hasDeferredRemoval())
					{
						if (pairCache.findPair(proxy0, proxy1) != null)
						{
							pairCache.removeOverlappingPair(proxy0, proxy1,
									dispatcher);
						}
					}
				}
