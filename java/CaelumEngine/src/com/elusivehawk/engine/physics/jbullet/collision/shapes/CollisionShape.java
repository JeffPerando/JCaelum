...
public abstract class CollisionShape
{
	...
	public abstract void getAabb(Transform t, Vector aabbMin, Vector aabbMax);

	public void getBoundingSphere(Vector center, float[] radius)
	{
		...
		Vector aabbMin = Stack.alloc(new Vector(3)),
				aabbMax = Stack.alloc(new Vector(3));
	...
	public float getAngularMotionDisc()
	{
		...
		disc[0] += Math.sqrt(center.length());
		return disc[0];
	}

	///calculateTemporalAabb calculates the enclosing aabb for the moving object over interval [0..timeStep)
	///result is conservative
	public void calculateTemporalAabb(Transform curTrans, Vector linvel, Vector angvel, float timeStep, Vector temporalAabbMin, Vector temporalAabbMax) {
		//start with static aabb
		getAabb(curTrans, temporalAabbMin, temporalAabbMax);

		float temporalAabbMaxx = temporalAabbMax.get(X);
		float temporalAabbMaxy = temporalAabbMax.get(Y);
		float temporalAabbMaxz = temporalAabbMax.get(Z);
		float temporalAabbMinx = temporalAabbMin.get(X);
		float temporalAabbMiny = temporalAabbMin.get(Y);
		float temporalAabbMinz = temporalAabbMin.get(Z);

		// add linear motion
		Vector linMotion = Stack.alloc(linvel);
		linMotion.scale(timeStep);

		//todo: simd would have a vector max/min operation, instead of per-element access
		for (int c = 0; c < linMotion.getSize(); c++)
		{
			if (linMotion.get(c) > 0f)
			{
				temporalAabbMaxx += linMotion.get(c);
				
			}
			else
			{
				temporalAabbMinx += linMotion.get(c);
		...
		Vector angularMotion3d = Stack.alloc(new Vector(3));
...
//#ifndef __SPU__
	...
	public abstract void calculateLocalInertia(float mass, Vector inertia);
