...
public class Dbvt
{
	...
	public static void collideTV(Node root, DbvtAabbMm volume, ICollide policy)
	{
		...
		if (root != null)
		{
			ObjectArrayList<Node> stack = new ObjectArrayList<Node>(
					SIMPLE_STACKSIZE);
			stack.add(root);
	...
	public static void collideRAY(Node root, Vector origin, Vector direction, ICollide policy)
	{
		...
		if (root != null)
		{
			...
			invdir.set(1f / normal.get(X), 1f / normal.get(Y),
					1f / normal.get(Z));
			int[] signs = new int[] { direction.get(X) < 0 ? 1 : 0,
					direction.get(Y) < 0 ? 1 : 0, direction.get(Z) < 0 ? 1 : 0 };
			ObjectArrayList<Node> stack = new ObjectArrayList<Node>(
					SIMPLE_STACKSIZE);
			stack.add(root);
		...
		if (root != null)
		{
			...
			do
			{
				...
				for (int i = 0, j = 1; (!out) && (i < count); ++i, j <<= 1)
				{
					if (0 == (se.mask & j))
					{
						int side = se.node.volume.Classify(normals[i],
								offsets[i], signs[i]);
						switch (side)
						{
							case -1:
	...
	public static void collideOCL(Node root, Vector[] normals, float[] offsets, Vector sortaxis, int count, ICollide policy, boolean fullsort)
	{
		...
		if (root != null)
		{
			...
			stack.add(allocate(ifree, stock,
					new sStkNPS(root, 0, root.volume.ProjectMinimum(sortaxis, srtsgns))));
			do
			{
				// JAVA NOTE: check
	...
	private static float size(DbvtAabbMm a)
	{
		...
		return (edges.get(X) * edges.get(Y) * edges.get(Z) + edges.get(X)
				+ edges.get(Y) + edges.get(Z));
	}
	...
	private static Vector[] axis = new Vector[] { new Vector(1, 0, 0),
	...
	private static Node topdown(Dbvt pdbvt, ObjectArrayList<Node> leaves, int bu_treshold)
	{
		if (leaves.size() > 1)
		{
			if (leaves.size() > bu_treshold)
			{
				...
				int[][] splitcount = new int[/* 3 */][/* 2 */] { { 0, 0 },
						{ 0, 0 }, { 0, 0 } };
